...
def login_user(username, password):
  ...
  if user and user.check_password(password): #if we have the user and the correct hash of password
    token = create_access_token(identity=username) #json-based end point | always return a javascrpt object of a dictionary in response
    response = jsonify(access_token=token) #hence jsonifying the response
    set_access_cookies(response, token) #automatically store in the browser | make easier to login
    return response
